...
#define START_MENU_ANIMATION_DURATION 3 // in seconds
#ifdef DEBUG_ACTIVE
#define CUTSCENE_DURATION 0.1       // in seconds
#else
#define CUTSCENE_DURATION 1.7       // in seconds
#endif
...
void on_pause_screen(){
	...
	write_text_aligned("< Pausado >", SCREEN_H * 0.2, 7, WHITE, CENTERED);
	BUTTON continue_b;
	default_blue(&continue_b, 0.50);
	TextCopy(continue_b.text, "Continuar");
	draw_button(continue_b);
	...
	if(check_button_click(continue_b) || pause_pressed) current_screen = GAMEPLAY_SCREEN;
...
void on_defeated_screen(){
	...
	BeginTextureMode(pixelated_screen);
	ClearBackground(BLANK);
	DrawTexture(defeat_screen, to_pixel_grid(SCREEN_W * 0.5 - CAMERA_ZOOM_FACTOR * 40 * PIXEL_PER_UNITS) / CAMERA_ZOOM_FACTOR, to_pixel_grid(SCREEN_H * 0.4 - CAMERA_ZOOM_FACTOR * 32* PIXEL_PER_UNITS) / CAMERA_ZOOM_FACTOR, WHITE);
	EndTextureMode();

	BeginDrawing();
	ClearBackground(VERY_VERY_DARK_PURPLE);
	write_text_aligned("< Fim de Jogo! >", SCREEN_H * 0.04, 7, WHITE, CENTERED);
	
	if(should_write_name){
		write_text_aligned("Boa pontuação!", SCREEN_H * 0.68, 7, WHITE, CENTERED);
		write_text_aligned("Escreva seu nome:", SCREEN_H * 0.78, 7, WHITE, CENTERED);
		...
		write_text_aligned(display_name, to_pixel_grid(SCREEN_H * 0.88), 7, WHITE, CENTERED);
		
	}
	else {
		write_text_aligned("\"Essa foi por pouco!\"", SCREEN_H * 0.68, 7, GBLUE, CENTERED);
		//write_text_aligned("", SCREEN_H * 0.78, 10, WHITE);
		BUTTON b;
...
void write_text_aligned(char *string, float vertical_pos, int font_size, Color color, POS_ON_SCREEN pos){
	BeginTextureMode(pixelated_screen);
	int text_width = MeasureText(string, font_size * UNITS_PER_PIXEL);
	float horizontal_position = SCREEN_W / CAMERA_ZOOM_FACTOR * 0.5;

	switch(pos){
		case RIGHT_ALIGNED: horizontal_position = 2 * horizontal_position - text_width - font_size; break;
		case CENTERED     : horizontal_position -= 0.5 * text_width; break;
		case LEFT_ALIGNED : horizontal_position = font_size; break; // Font_size acts as padding on left and right alignment
	}
	DrawText(string, 
		to_pixel_grid(UNITS_PER_PIXEL * horizontal_position), 
		to_pixel_grid(UNITS_PER_PIXEL * vertical_pos / CAMERA_ZOOM_FACTOR), 
		font_size * UNITS_PER_PIXEL, 
		color);
	EndTextureMode();
}
...
void on_score_screen(){
	...
	float score_f_size = 7;
	float score_line_size = 0.5 * SCREEN_H / score_f_size;
...
void draw_button(BUTTON button){
	...
	box_in_grid.x *= UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR;
	box_in_grid.y *= UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR;
	
	DrawRectangle(box_in_grid.x + 2, box_in_grid.y + 2, box_in_grid.width, box_in_grid.height, button.shadow_color); // Draw a "shadow"
	
	Vector2 mouse_position = (Vector2){GetMousePosition().x * UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR, GetMousePosition().y  * UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR};
	...
	DrawText(button.text, 
			...
			to_pixel_grid(box_in_grid.y + 0.5 * box_in_grid.height - button.font_size * UNITS_PER_PIXEL + 2), 
			button.font_size, 
...
void default_button(BUTTON *b, float vertical_pos_as_perc){
	b->box = (Rectangle){SCREEN_W * 0.5 - 43 * UNITS_PER_PIXEL * CAMERA_ZOOM_FACTOR, SCREEN_H * vertical_pos_as_perc, 86, 12};
	b->font_size = 7;
	b->text_color = WINE;
...
Rectangle draw_offset_menu(Texture2D layer_texture, float scalar, float screen_scalar, bool darker){
	...
	Rectangle dest_screen = (Rectangle){	screen_scalar * screen_offset * PIXEL_PER_UNITS * MAP_SIZE_IN_UNITS / CAMERA_ZOOM_FACTOR, 
											screen_scalar *screen_offset * PIXEL_PER_UNITS * MAP_SIZE_IN_UNITS / CAMERA_ZOOM_FACTOR,
											200 * screen_scalar * 4 * UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR,
											128 * screen_scalar * 4 * UNITS_PER_PIXEL / CAMERA_ZOOM_FACTOR};
