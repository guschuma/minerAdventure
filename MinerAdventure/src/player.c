...
#define LITTLE_DASH_FORCE (0.3)		// Happens when player has not pressed upwards // 1.2
...
#define CLIMBING_ANIMATION_CHANGE_TIME (0.4)
...
Texture2D player_climbing_sprite;
...
float old_y_velocity;

bool is_player_on_ladder;
float animation_climbing_time = 0;
bool flipped_climbing_sprite;
bool debug_climbing = false;
...
Vector2 calculate_player_hook_velocity(Vector2 current_velocity){
	...
	bool hook_was_moving = hook.is_moving;
	update_hook_position();
	if(!hook.is_active) return current_velocity;
	if((IsMouseButtonDown(MOUSE_BUTTON_LEFT) || IsMouseButtonDown(MOUSE_BUTTON_RIGHT)) && !hook.is_moving){
		if(hook_was_moving) play_sound(HOOK_LAND_SFX);
		Vector2 hook_center = {
...
void update_player_position(){
	...
	is_moving = false;
	#ifdef DEBUG_ACTIVE
	if(IsKeyPressed(KEY_H)) debug_climbing = !debug_climbing;
	if(debug_climbing) is_player_on_ladder = true;
	#endif
	...
	if(isPressed(UPK) && player.is_grounded && !player.was_hit_animation){
		...
		if(CheckCollisionRecs(player.body.hitbox, end_elevator_rec) && active_enemy_count == 0) exit_level_cutscene();
		else play_sound(JUMP_SFX);
	}
	else if(isPressed(UPK) && is_player_on_ladder && !player.was_hit_animation){
		current_velocity.y -= PLAYER_ACCEL * DT; // Player climbing velocity is same as walking velocity
	}
	if(isPressed(DOWNK) && is_player_on_ladder && !player.was_hit_animation){
		current_velocity.y += PLAYER_ACCEL * DT; // Player climbing velocity is same as walking velocity
	}
	...
	is_player_on_ladder &= !player.was_hit_animation;
	if(!player.is_grounded && !is_player_on_ladder) current_velocity.y += GRAVITY_ACCEL * DT;
	if(is_player_on_ladder) {
		current_velocity = Vector2Scale(current_velocity, 0.9);
		if(!on_dash_delay) player_will_be_allowed_to_dash();
	}
	...
	new_position.x = collision_detect_blocks(&current_velocity.x, HORIZO, &player.body, &(player.is_grounded), &took_damage, true);
	Vector2 temp_new_position; // To update x axis first
	temp_new_position.x = new_position.x;
	temp_new_position.y =player.body.hitbox.y;
	update_body_position(temp_new_position, &player.body);
	new_position.y = collision_detect_blocks(&current_velocity.y, VERTIC, &player.body, &(player.is_grounded), &took_damage, true);
	if(took_damage) player_was_hit();
	if(current_velocity.y != old_y_velocity && current_velocity.y == 0) {
		play_sound(FOOTSTEP); // "Landing" sound
	}
	player.body.velocity_vector = current_velocity;
	update_body_position(new_position, &player.body);
	if(player.body.position.y > MAP_SIZE_IN_UNITS + 10 * UNITS_PER_BLOCK) { // Player fell off map
		player_was_hit();
	}
}
...
void update_player_texture(){
	...
	if(player.was_hit_animation){
		...
		if(is_player_on_ladder){
			player.texture = player_climbing_sprite;
			player.texture_rect = flipped_climbing_sprite? (Rectangle){0, 0, -16, 16} : (Rectangle){0, 0, 16, 16};
			if(is_moving || isPressed(UPK) || isPressed(DOWNK))animation_climbing_time += GetFrameTime();
			if(animation_climbing_time > CLIMBING_ANIMATION_CHANGE_TIME) {
				animation_climbing_time = 0;
				play_sound(LADDER_CLIMBING); 
				flipped_climbing_sprite = !flipped_climbing_sprite;
			}
		}
		else{
			player.texture = player_jumping_sprites;
		...
		if(is_walk_animation_timer_active()){
			frame_index++;
			frame_index %= 8;
			if(frame_index % 4 == 0) play_sound(FOOTSTEP);
		}
...
void draw_lifes(int health){
	...
	first_life_position.x += PIXELS_PER_BLOCK;
	if(health == 4) DrawTextureRec(player_life_sprite, source_rec, first_life_position, GBLUE);
	EndTextureMode();
...
void update_player_collision_enemy(Enemy *e){
	...
	player_vertical_position += player.body.hitbox.height * 0.3;
	if(player_vertical_position < enemy_vertical_position && player_vertical_velocity >= 0){
		...
		if(e->type != ENEMY_BAT && e->type != ENEMY_FROG) player.body.hitbox.y = enemy_vertical_position - player.body.hitbox.height;
		player.body.velocity_vector.y = -PLAYER_STOMP_VELOCITY * player.velocity;
	}
	else {
		if(!e->was_hit)  player_was_hit();
	}
}
