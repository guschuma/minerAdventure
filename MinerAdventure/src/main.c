...
char debug_block; // Only using during debug for level making
...
void update_gameplay_loop(void){
	...
	if(IsKeyPressed(KEY_R)){
		...
		mouse_position = v_camera_to_global(mouse_position);
		int j = mouse_position.x / UNITS_PER_BLOCK - BORDER_SIZE_W;
		int i = mouse_position.y / UNITS_PER_BLOCK - BORDER_SIZE_H;
		if(IsMouseButtonDown(MOUSE_BUTTON_LEFT))mapa[i][j] = debug_block;
		else  mapa[i][j] = ' ';
	...
	draw_textured_screen(shadows, true);
	draw_textured_screen(shadows, true);
	//ClearBackground(WHITE);
	//draw_textured_screen(shadows, true);
...
void update_music(SCREENS old_screen){
	if(old_screen == current_screen){
		...
		case DEFEAT_SCREEN:
			UpdateMusicStream(defeat_music);
			break;
